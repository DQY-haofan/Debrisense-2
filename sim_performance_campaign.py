...
# 版本: 2.1 (修复 CSV 路径问题)
# 描述:
#   生成 Fig 7 (ROC 曲线) 和 Fig 8 (速度盲区分析)。
#   使用 joblib 进行 CPU 多核并行加速。
# ----------------------------------------------------------------------------------
import numpy as np
...
print(f"[INFO] Using {n_jobs} cores for advanced metrics calculation.")
...
def trial_roc(is_h1, seed, noise_std, phy_engine, true_v):
    ...
    if is_h1:
        ...
        d_wb = phy_engine.generate_broadband_chirp(t_axis, N_sub=32)
        sig = 1.0 + d_wb
    else:
...
def run_fig7_roc():
    ...
    # Run H0 (Noise Only)
    res0 = Parallel(n_jobs=n_jobs)(delayed(trial_roc)(False, s, noise_std, phy, true_v) for s in seeds)
    res0 = np.array(res0)

    # Run H1 (Signal + Noise)
    res1 = Parallel(n_jobs=n_jobs)(delayed(trial_roc)(True, s, noise_std, phy, true_v) for s in seeds)
    res1 = np.array(res1)
    ...
    print("Saved results/Fig7_ROC.csv")
...
def run_fig8_velocity():
    ...
    h0_stats = Parallel(n_jobs=n_jobs)(delayed(trial_roc)(False, s, noise_std, phy_dummy, 15000) for s in seeds)
    h0_scores = np.array(h0_stats)[:, 0]
    threshold = np.percentile(h0_scores, 99.9)
    print(f"[INFO] Threshold (Pfa=1e-3): {threshold:.4f}")
    ...
    print("Saved results/Fig8_Velocity.csv")
